...
#include "./data/a5200_background.h"
#include "./data/a5200_load.h"
...
#define COLOR_INACTIVE_ITEMD PIX_TO_RGB(layer->format,119,138,195)
...
MENUITEM MainMenuItems[] = {
	...
	{"Analog Stick: ",  (int *) &GameConf.m_Analog, 1,(char *) &mnuYesNo, NULL},
...
void screen_prepbackground(void) {
	...
	screen_prepback(layerbackgrey, A5200_BACKGROUND, A5200_BACKGROUND_SIZE);
}
...
/*
void screen_loadicons(unsigned char *bmpBuf, unsigned int bmpSize) {
	// load icons, Convert the image to optimal display format and Free the temporary surface
	if (layericons) SDL_FreeSurface(layericons);
	SDL_RWops *rw = SDL_RWFromMem(bmpBuf, bmpSize);
	SDL_Surface *temp = SDL_LoadBMP_RW(rw, 1);
	layericons = SDL_DisplayFormat(temp);
	SDL_FreeSurface(temp);
}
*/
...
void screen_showtopmenu(void) {
	...
	//screen_loadicons(STELLA_BOTTOM, STELLA_BOTTOM_SIZE);
	...
	atari_analog = GameConf.m_Analog;
}
...
signed int load_file(char **wildcards, char *result) {
	...
	while (return_value == 1) {
		...
		while(repeat) {
			...
			screen_prepback(layer, A5200_LOAD, A5200_LOAD_SIZE);
...
#define CHARLEN ((320/6)-2)
				if(current_filedir_number < num_filedir) {
					...
					if((current_filedir_number == current_filedir_selection)) {
						...
						print_string(print_buffer, filedir_list[current_filedir_number].type == 0 ? COLOR_INACTIVE_ITEM : COLOR_INACTIVE_ITEMD, COLOR_BG, 4,10*3+ ((i + 2) * 8));
					}
				}
			}
...
char *file_ext[] = { (char *) ".a52", (char *) ".bin", NULL };
  ...
  if (fd >= 0) {
	...
	atari_analog = GameConf.m_Analog;
	  ...
	  //  0,    1,    2,    3, 4, 5, 4, 5, 4, 5,     6,      6
		...
		GameConf.m_Analog = 0; // 0 = no
		GameConf.m_Scanline = 0; // 0 = no
